...
function drawArrow(
  ...
  const ux = dx / len;
  const uy = dy / len;
  const padding = CITY_RADIUS + 10;
  const sx = x1 + ux * padding;
  const sy = y1 + uy * padding;
  const ex = x2 - ux * padding;
  const ey = y2 - uy * padding;
  ...
  ctx.shadowBlur = 8;
  // Animated marching dashes (offset by current time / 100ms).
  const phase = (Date.now() / 60) % 16;
  ctx.setLineDash([8, 6]);
  ...
  // ── Marching banner — flag drifting along the route ──
  // Position cycles 0→1 over ~3 seconds; flag faces forward, rippling.
  const t = (Date.now() / 2400) % 1;
  const bx = sx + (ex - sx) * t;
  const by = sy + (ey - sy) * t;
  ctx.save();
  ctx.translate(bx, by);
  ctx.rotate(angle);
  // Pole — vertical (after rotation, perpendicular to march dir).
  ...
  ctx.lineTo(0, -11);
  ctx.stroke();
  // Flag triangle — ripples with sin
  const ripple = Math.sin(Date.now() / 120) * 1.8;
  ctx.fillStyle = color;
  ctx.shadowColor = 'rgba(0,0,0,0.55)';
  ctx.shadowBlur = 3;
  ctx.beginPath();
  ctx.moveTo(0, -11);
  ctx.lineTo(9 + ripple, -8);
  ctx.lineTo(0, -5);
  ctx.closePath();
  ctx.fill();
  ctx.restore();
